...
module FixMyTransport
  ...
  module Email
    ...
    class Mail < TMail::Mail
      ...
      def friendly_from_or_sender( default = nil )
        if self.from
          return self.friendly_from if self.friendly_from
        end
        if self.sender
          return self.friendly_sender if self.friendly_sender
        end
        return default
      end

      def friendly_sender( default = nil )
        h = @header['sender']
      ...
      def from_or_sender_address
        if self.from_addrs && self.from_addrs.size > 0
          return self.from_addrs[0].address
        end
        if self.sender
          return self.sender
        end
