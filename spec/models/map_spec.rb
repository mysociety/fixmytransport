  ...
  describe 'when asked for other locations' do
    ...
    describe 'when the current zoom level is >= the minimum zoom level for showing other markers' do
      ...
      describe 'if no highlight param is passed' do
        ...
        it 'should ask for current stops and stop areas in the bounding box passing an empty list of ids to exclude' do
          expected_options = { :exclude_ids => [] }
          expected_params = [anything(), expected_options]
          [Stop, StopArea].each do |model|
            model.should_receive(:find_current_in_bounding_box).with(*expected_params).and_return([])
          end
      ...
      describe 'if a highlight param is passed' do
        ...
        it 'should ask for current stops and stop areas in the bounding box, passing a list of ids to exclude
            from the issues' do
          ...
          expected_params = [anything(), expected_options]
          Stop.should_receive(:find_current_in_bounding_box).with(*expected_params).and_return([])

          expected_options = { :exclude_ids => [44] }
          expected_params = [anything(), expected_options]
          StopArea.should_receive(:find_current_in_bounding_box).with(*expected_params).and_return([])
          Map.other_locations(51.505720, -0.088872, MIN_ZOOM_FOR_OTHER_MARKERS, MAP_HEIGHT, MAP_WIDTH, highlight=:has_content)
        end
