...
describe Assignment do
  ...
  describe 'when completing' do
    ...
    describe 'when validating assignments' do
      ...
      describe 'an assignment to write to a new transport organization' do

        before do
          @route = mock_model(Route)
          @problem = mock_model(Problem, :location => @route)
          @campaign = mock_model(Campaign, :problem => @problem)
          ...
          @assignment = Assignment.new(:task_type_name => 'write-to-new-transport-organization',
                                       :campaign => @campaign,
                                       :problem => @problem )
        end

        it 'should be invalid without an organization name' do
          @assignment.data = { :organization_type => 'Operator',
                               :organization_persistent_id => 77 }
          @assignment.valid?.should be_false
          @assignment.errors.on(:organization_name).should == "Please give the company name"
        end

        it 'should be invalid without an organization type' do
          @assignment.data = { :organization_name => 'A test organization',
                               :organization_persistent_id => 77 }
          @assignment.valid?.should be_false
          @assignment.errors.on(:organization_type).should == "Please give the type of the organization"
        end

        it 'should be invalid without an organization persistent id' do
          @assignment.data = { :organization_name => 'A test organization',
