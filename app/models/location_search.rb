...
class LocationSearch < ActiveRecord::Base
  ...
  def self.new_search!(session_id, params)
    ...
    attributes = params[:campaign][:location_attributes]
    attributes[:transport_mode_id] = params[:campaign][:transport_mode_id]
    attributes[:location_type] = params[:campaign][:location_type]
